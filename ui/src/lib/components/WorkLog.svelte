        ...
        >
            <div class="flex flex-col w-full">
                <div class="flex flex-row gap-2 w-full justify-between my-2 px-2">
                    <p class="text-sm">{ formatDateTime(log.startedAt) }</p>
                    <p class="text-sm text-gray-500">{ log.duration } mins</p>
                </div>

                <div class="w-full h-[1px] bg-gray-200">&nbsp;</div>

                <p class="px-2 my-2 text-md">{log.description}</p>
                ...
                {#if log.collaborators.length > 0}
                    ...
                    <div class="w-full flex flex-row-reverse flex-wrap">
                        {#each log.collaborators as collaborator (collaborator.id)}
