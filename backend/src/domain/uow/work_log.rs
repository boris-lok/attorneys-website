...
impl<F: UnitOfWorkFactory> WorkLogUoW<F> {
    ...
    pub async fn update(
        ...
        if log.is_collaborative && touches_protected {
            return Err(WorkLogError::PermissionDenied);
        }

        if req.status.is_some() && log.is_collaborative {
            uow.work_log_mapping_repo()
